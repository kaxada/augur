...
def ping_github_for_repo_move(augur_db, key_auth, repo, logger,collection_hook='core'):
    ...
    if response_from_gh.status_code == 404:
        ...
        repo_update_dict = {
            ...
            'repo_path': None,
            'repo_name': None,
            'description': f"During our check for this repo on {datetime.now().strftime('%Y-%m-%d')}, a 404 error was returned. The repository does not appear to have moved. Instead, it appears to be deleted",
            'data_collection_date': datetime.now().strftime(
                '%Y-%m-%dT%H:%M:%SZ'
            ),
        }
    ...
    if collection_hook == 'core':
        ...
        collectionRecord.core_data_last_collected = datetime.now().strftime(
            '%Y-%m-%dT%H:%M:%SZ'
        )
        ...
        collectionRecord.secondary_data_last_collected = (
