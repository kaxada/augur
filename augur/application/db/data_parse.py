...
def extract_pr_review_message_ref_data(comment: dict, augur_pr_review_id, github_pr_review_id, repo_id: int, tool_version: str, data_source: str) -> dict:
    ...
    return {
        ...
        'pr_review_msg_position': s.sql.expression.null()
        if not (  # This had to be changed because "None" is JSON. SQL requires NULL SPG 11/28/2021
            # 12/6/2021 - removed casting from value check
            comment['position']
        )
        else comment['position'],
        'pr_review_msg_original_position': s.sql.expression.null()
        if not (  # This had to be changed because "None" is JSON. SQL requires NULL SPG 11/28/2021
            # 12/6/2021 - removed casting from value check
        ...
        'pr_review_msg_start_line': s.sql.expression.null()
        if not (  # This had to be changed because "None" is JSON. SQL requires NULL SPG 11/28/2021
            # 12/6/2021 - removed casting from value check
            comment['start_line']
        )
        else comment['start_line'],
        'pr_review_msg_original_start_line': s.sql.expression.null()
        if not (  # This had to be changed because "None" is JSON. SQL requires NULL SPG 11/28/2021
            # 12/6/2021 - removed casting from value check
            comment['original_start_line']
        )
        else int(comment['original_start_line']),
        'pr_review_msg_start_side': s.sql.expression.null()
        if not (  # This had to be changed because "None" is JSON. SQL requires NULL SPG 11/28/2021
            str(comment['start_side'])
        )
        else str(comment['start_side']),
        'pr_review_msg_line': s.sql.expression.null()
        if not (  # This had to be changed because "None" is JSON. SQL requires NULL SPG 11/28/2021
            # 12/6/2021 - removed casting from value check
            comment['line']
        )
        else int(comment['line']),
        'pr_review_msg_original_line': s.sql.expression.null()
        if not (  # This had to be changed because "None" is JSON. SQL requires NULL SPG 11/28/2021
            # 12/6/2021 - removed casting from value check
            comment['original_line']
        )
        else int(comment['original_line']),
        'pr_review_msg_side': s.sql.expression.null()
        if not (  # This had to be changed because "None" is JSON. SQL requires NULL SPG 11/28/2021
            str(comment['side'])
        )
        else str(comment['side']),
        'tool_source': 'pull_request_reviews model',
        'tool_version': f"{tool_version}_reviews",
        'data_source': data_source,
        'repo_id': repo_id,
    }
...
def extract_pr_event_data(event: dict, pr_id: int, platform_id: int, repo_id: int, tool_source: str, tool_version: str, data_source: str) -> dict:
    ...
    return {
        ...
        'cntrb_id': event.get("cntrb_id", None),
        'action': event['event'],
...
def extract_issue_event_data(event: dict, issue_id: int, platform_id: int, repo_id: int, tool_source: str, tool_version: str, data_source: str) -> dict:
    ...
    return {
        ...
        'node_url': event['url'],
        'cntrb_id': event.get("cntrb_id", None),
        'created_at': event['created_at'] if (event['created_at']) else None,
        'action': event['event'],
...
def extract_needed_pr_data(pr, repo_id, tool_source, tool_version):
    ...
    return {
        ...
        'pr_src_id': int(
            str(pr['id']).encode(encoding='UTF-8').decode(encoding='UTF-8')
        ),
        ...
        'pr_closed_at': None if not (pr['closed_at']) else pr['closed_at'],
        'pr_merged_at': None if not (pr['merged_at']) else pr['merged_at'],
        ...
        'pr_meta_head_id': None if not (pr['head']) else pr['head']['label'],
        'pr_meta_base_id': None if not (pr['base']) else pr['base']['label'],
...
def extract_needed_issue_data(issue: dict, repo_id: int, tool_source: str, tool_version: str, data_source: str):
    ...
    return {
        ...
        'reporter_id': issue[
            "cntrb_id"
        ],  # this is the contributor who opened the issue
        'pull_request': None,
...
def extract_needed_pr_review_data(review, pull_request_id, repo_id, platform_id, tool_version, data_source):
    ...
    return {
        ...
        'pr_review_body': str(review['body'])
        .encode(encoding='UTF-8', errors='backslashreplace')
        .decode(encoding='UTF-8', errors='ignore')
        if (review['body'])
        else None,
